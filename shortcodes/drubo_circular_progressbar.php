...
add_action('init', 'drubo_circular_progressbar_map'); // Call kc_add_map function ///

if(!function_exists('drubo_circular_progressbar_map')):
	function drubo_circular_progressbar_map(){
		if(function_exists('kc_add_map')): // if kingComposer is active
		kc_add_map(
		    array(
		        'drubo_circular_progressbar' => array( // <-- shortcode tag name

		            'name' => __('Circular Progressbar', DRUBO_SHORTCODE_TEXT_DOMAIN),
...
function drubo_circular_progressbar_func($atts,$content){
ob_start();
...
add_shortcode('drubo_circular_progressbar','drubo_circular_progressbar_func');